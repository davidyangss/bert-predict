/// From git@github.com:pykeio/ort.git, branch = 2.0.0-rc.4, file = examples/training/examples/pretokenize.rs
...
fn main() {
    ...
    for line in input.lines() {
        let line: simd_json::OwnedValue =
            unsafe { simd_json::from_str(&mut line.unwrap()).unwrap() };
        let tokenized = tokenizer
            .encode(
                format!("<|endoftext|>{}", line.get_str("message").unwrap()),
                false,
            )
            .unwrap();
