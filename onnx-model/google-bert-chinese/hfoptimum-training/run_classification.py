...
class DataTrainingArguments:
    ...
    csv_column_delimiter: Optional[str] = field(
        default=",",
        metadata={
            "help": "THe delimiter is column delimiter in csv."
        },
    )
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, ORTTrainingArguments)
    )
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file.endswith(".csv"):
            ...
            raw_datasets = load_dataset(
                ...
                cache_dir=model_args.cache_dir,
                token=model_args.token,
                delimiter=data_args.csv_column_delimiter,
                quoting=3,
            )
            logger.info(f"Dataset(delimiter={data_args.csv_column_delimiter}) loaded: {raw_datasets}")

            # for split in raw_datasets.keys():
            #     current_features = raw_datasets[split].features
            #     current_features[data_args.label_column_name] = Value('float32')
            #     raw_datasets[split] = raw_datasets[split].cast(current_features)
        else:
    ...
    def preprocess_function(examples):
        #logger.info(f"preprocess_function = {examples}")
